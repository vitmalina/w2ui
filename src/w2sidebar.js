...
(function () {
    ...
    $.fn.w2sidebar = function (method) {
        if (typeof method === 'object' || !method) {
            ...
            if (!$.fn.w2checkNameParam(method, 'w2sidebar'))
                return;
            // extend items
            var nodes = method.nodes,
                object = new w2sidebar(method);
    ...
    w2sidebar.prototype = {
        ...
        remove: function () { // multiple arguments
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (ind === null)
                    continue;
                if (tmp.parent.nodes[ind].selected)
                    tmp.sidebar.unselect(tmp.id);
                tmp.parent.nodes.splice(ind, 1);
                deleted++;
            }
            if (deleted > 0 && arguments.length === 1) {
                this.refresh(tmp.parent.id);
            } else {
                this.refresh();
            }
            return deleted;
        ...
        collapse: function (id) {
            var nd = this.get(id),
                // event before
                eventData = this.trigger({phase: 'before', type: 'collapse', target: id, object: nd});
        ...
        collapseAll: function (parent) {
            ...
            for (var i = 0; i < parent.nodes.length; i++) {
                if (parent.nodes[i].expanded === true) {
                    parent.nodes[i].expanded = false;
                }
                if (parent.nodes[i].nodes && parent.nodes[i].nodes.length > 0) {
                    this.collapseAll(parent.nodes[i]);
                }
            }
            this.refresh(parent.id);
        },
        ...
        expandAll: function (parent) {
            ...
            for (var i = 0; i < parent.nodes.length; i++) {
                if (parent.nodes[i].expanded === false) {
                    parent.nodes[i].expanded = true;
                }
                if (parent.nodes[i].nodes && parent.nodes[i].nodes.length > 0) {
                    this.collapseAll(parent.nodes[i]);
                }
        ...
        click: function (id, event) {
            ...
            if (nd.disabled || nd.group) {
                return; // should click event if already selected
            }
            // move selected first
        ...
        keydown: function (event) {
            var obj = this,
                nd = obj.get(obj.selected);
            if (!nd || obj.keyboard !== true) {
                return;
            }
            // trigger event
            ...
            if (event.keyCode === 39) { // right
                if ((nd.nodes.length > 0 || nd.plus) && !nd.expanded) {
                    obj.expand(obj.selected);
                }
            }
            ...
            function next (node, noSubs) {
                ...
                var parent = node.parent,
                    ind = obj.get(node.id, true),
                    nextNode = null;
                // jump inside
                if (node.expanded && node.nodes.length > 0 && noSubs !== true) {
                    var t = node.nodes[0];
                    if (t.hidden || t.disabled || t.group) {
                        nextNode = next(t);
                    } else {
                        nextNode = t;
                    }
                ...
                if (nextNode !== null && (nextNode.hidden || nextNode.disabled || nextNode.group)) {
            ...
            function prev (node) {
                ...
                var parent = node.parent,
                    ind = obj.get(node.id, true),
                    prevNode = (ind > 0) ? lastChild(parent.nodes[ind - 1]) : parent;
                if (prevNode !== null && (prevNode.hidden || prevNode.disabled || prevNode.group))
        ...
        scrollIntoView: function (id) {
            ...
            var body = $(this.box).find('.w2ui-sidebar-div'),
                item = $(this.box).find('#node_' + w2utils.escapeId(id)),
                offset = item.offset().top - body.offset().top;
        ...
        menuClick: function (itemId, index, event) {
            var obj = this,
                // event before
                eventData = obj.trigger({phase: 'before', type: 'menuClick', target: itemId, originalEvent: event, menuIndex: index, menuItem: obj.menu[index]});
            if (eventData.isCancelled === true) {
                return false;
            }
            // default action
        ...
        refresh: function (id) {
            ...
            function getNodeHTML (nd) {
                ...
                if (typeof nd.caption !== 'undefined')
                    nd.text = nd.caption;
                if (nd.group) {
                    html =
                    ...
                    tmp = '';
                    if (img)
                        tmp = '<div class="w2ui-node-image w2ui-icon ' + img + (nd.selected && !nd.disabled ? " w2ui-icon-selected" : "") + '"></div>';
                    if (icon)
