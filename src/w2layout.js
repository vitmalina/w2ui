...
(function () {
    ...
    $.fn.w2layout = function (method) {
        if (typeof method === 'object' || !method) {
            ...
            if (!$.fn.w2checkNameParam(method, 'w2layout'))
                return;
            var panels = method.panels,
                object = new w2layout(method);
            $.extend(object, {handlers: [], panels: []});
            // add defined panels panels
            for (var p in panels) {
                object.panels[p] = $.extend(true, {}, w2layout.prototype.panel, panels[p]);
                if ($.isPlainObject(object.panels[p].tabs) || $.isArray(object.panels[p].tabs))
                    initTabs(object, panels[p].type);
                if ($.isPlainObject(object.panels[p].toolbar) || $.isArray(object.panels[p].toolbar))
                    initToolbar(object, panels[p].type);
            }
            // add all other panels
            for (var p1 in {'top': '', 'left': '', 'main': '', 'preview': '', 'right': '', 'bottom': ''}) {
                if (object.get(p1) !== null)
                    continue;
        ...
        function initTabs (object, panel, tabs) {
            ...
            if (pan !== null && typeof tabs === 'undefined')
                tabs = pan.tabs;
            if (pan === null || tabs === null)
                return false;
            // instanciate tabs
            if ($.isArray(tabs))
                tabs = {tabs: tabs};
        ...
        function initToolbar (object, panel, toolbar) {
            var pan = object.get(panel);
            if (pan !== null && typeof toolbar === 'undefined')
                toolbar = pan.toolbar;
            if (pan === null || toolbar === null)
                return false;
            // instanciate toolbar
            if ($.isArray(toolbar))
    ...
    w2layout.prototype = {
        ...
        content: function (panel, data, transition) {
            ...
            if (p === null)
                return false;
            if ($('#layout_' + this.name + '_panel2_' + p.type).length > 0)
                return false;
            ...
            if (data === null || typeof data == 'undefined') {
                ...
                if (tmp.length > 0 && typeof p.style != 'undefined')
                    tmp[0].style.cssText = p.style;
                if (p.content === '') {
                    p.content = data;
                    if (!p.hidden)
                        this.refresh(panel);
        ...
        show: function (panel, immediate) {
            var obj = this,
                // event before
                eventData = this.trigger({phase: 'before', type: 'show', target: panel, object: this.get(panel), immediate: immediate});
        ...
        toggle: function (panel, immediate) {
            ...
            if (p === null)
                return false;
            if (p.hidden)
                return this.show(panel, immediate);
            else
                return this.hide(panel, immediate);
        },
        ...
        render: function (box) {
            ...
            $(obj.box)
                ...
                .html('<div></div>');
            if ($(obj.box).length > 0)
                $(obj.box)[0].style.cssText += obj.style;
            // create all panels
            ...
            function resizeStop (evnt) {
                ...
                if (typeof obj.tmp.resize == 'undefined')
                    return;
                // unlock all panels
                var panels = ['left', 'right', 'top', 'bottom', 'preview', 'main'];
                for (var p in panels)
            ...
            function resizeMove (evnt) {
                ...
                switch (obj.tmp.resize.type) {
                    ...
                    case 'bottom':
                        ...
                        if (p.length > 0)
                            p[0].style.top = (obj.tmp.resize.value + obj.tmp.resize.div_y) + 'px';
                        break;
                    ...
                    case 'right':
                        ...
                        if (p.length > 0)
                            p[0].style.left = (obj.tmp.resize.value + obj.tmp.resize.div_x) + 'px';
                        break;
                }
                // event after
                obj.trigger($.extend(eventData, {phase: 'after'}));
        ...
        resize: function () {
            ...
            var obj = this,
                // panels
                pmain = this.get('main'),
                pprev = this.get('preview'),
                pleft = this.get('left'),
                pright = this.get('right'),
                ptop = this.get('top'),
                pbottom = this.get('bottom'),
                smain = true, // main always on
                sprev = (pprev !== null && pprev.hidden !== true ? true : false),
                sleft = (pleft !== null && pleft.hidden !== true ? true : false),
                sright = (pright !== null && pright.hidden !== true ? true : false),
                stop = (ptop !== null && ptop.hidden !== true ? true : false),
                sbottom = (pbottom !== null && pbottom.hidden !== true ? true : false),
                l, t, w, h, e;
            // calculate %
            for (var p in {'top': '', 'left': '', 'right': '', 'bottom': '', 'preview': ''}) {
                ...
                if (tmp.sizeCalculated < parseInt(tmp.minSize))
                    tmp.sizeCalculated = parseInt(tmp.minSize);
            }
            // top if any
            ...
            if (pleft !== null && pleft.hidden !== true) {
                ...
                if (window.navigator.userAgent.indexOf('MSIE') > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight)
                    w += 17; // IE hack
                e.css({
                    'display': 'block',
                    'left': l + 'px',
            ...
            for (var p1 in {'top': '', 'left': '', 'main': '', 'preview': '', 'right': '', 'bottom': ''}) {
                ...
                var tabHeight = 0;
                if (pan.show.tabs) {
                    if (pan.tabs !== null && w2ui[this.name + '_' + p1 + '_tabs'])
                        w2ui[this.name + '_' + p1 + '_tabs'].resize();
                    tabHeight += w2utils.getSize($(tmp2 + 'tabs').css({display: 'block'}), 'height');
                }
                if (pan.show.toolbar) {
                    if (pan.toolbar !== null && w2ui[this.name + '_' + p1 + '_toolbar'])
                        w2ui[this.name + '_' + p1 + '_toolbar'].resize();
        ...
        destroy: function () {
            ...
            if (eventData.isCancelled === true)
                return false;
            if (typeof w2ui[this.name] == 'undefined')
                return false;
            // clean up
            ...
            if (this.tmp.events && this.tmp.events.resize)
                $(window).off('resize', this.tmp.events.resize);
            if (this.tmp.events && this.tmp.events.mousemove)
                $(document).off('mousemove', this.tmp.events.mousemove);
            if (this.tmp.events && this.tmp.events.mouseup)
