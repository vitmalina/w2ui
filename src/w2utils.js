var w2ui = w2ui || {},
    w2obj = w2obj || {}; // expose object to be able to overwrite default functions
...
var w2utils = (function () {
    ...
    function isDate (val, format, retDate) {
        ...
        var tmp = val.replace(/-/g, '/').replace(/\./g, '/').toLowerCase().split('/'),
            tmp2 = format.replace(/-/g, '/').replace(/\./g, '/').toLowerCase(),
            dt = 'Invalid Date';
        var month, day, year;
        if (tmp2 === 'mm/dd/yyyy') {
            month = tmp[0];
    ...
    function isTime (val) {
        ...
        val = val.toUpperCase();
        if (val.indexOf('PM') >= 0 || val.indexOf('AM') >= 0)
            max = 12;
        else
            max = 23;
    ...
    function age (timeStr) {
        if (timeStr === '' || typeof timeStr === 'undefined' || timeStr === null)
            return '';
        if (w2utils.isInt(timeStr))
            timeStr = Number(timeStr); // for unix timestamps

        var d1 = new Date(timeStr);
        if (w2utils.isInt(timeStr))
            d1 = new Date(Number(timeStr)); // for unix timestamps
        var tmp = String(timeStr).split('-');
        if (tmp.length === 3)
            d1 = new Date(tmp[0], Number(tmp[1]) - 1, tmp[2]); // yyyy-mm-dd
        var tmp = String(timeStr).split('/');
        if (tmp.length === 3)
            d1 = new Date(tmp[2], Number(tmp[0]) - 1, tmp[1]); // mm/dd/yyyy
        if (d1 === 'Invalid Time')
            return '';
    ...
    function date (dateStr) {
        ...
        if (dateStr === '' || typeof dateStr === 'undefined' || dateStr === null)
            return '';
        if (w2utils.isInt(dateStr))
            dateStr = Number(dateStr); // for unix timestamps

        var d1 = new Date(dateStr);
        if (w2utils.isInt(dateStr))
            d1 = new Date(Number(dateStr)); // for unix timestamps
        ...
        var dd1 = months[d1.getMonth()] + ' ' + d1.getDate() + ', ' + d1.getFullYear(),
            dd2 = months[d2.getMonth()] + ' ' + d2.getDate() + ', ' + d2.getFullYear(),
            dd3 = months[d3.getMonth()] + ' ' + d3.getDate() + ', ' + d3.getFullYear(),
            time = (d1.getHours() - (d1.getHours() > 12 ? 12 : 0)) + ':' + (d1.getMinutes() < 10 ? '0' : '') + d1.getMinutes() + ' ' + (d1.getHours() >= 12 ? 'pm' : 'am'),
            time2 = (d1.getHours() - (d1.getHours() > 12 ? 12 : 0)) + ':' + (d1.getMinutes() < 10 ? '0' : '') + d1.getMinutes() + ':' + (d1.getSeconds() < 10 ? '0' : '') + d1.getSeconds() + ' ' + (d1.getHours() >= 12 ? 'pm' : 'am'),
            dsp = dd1;
        if (dd1 === dd2)
            dsp = time;
        if (dd1 === dd3)
            dsp = w2utils.lang('Yesterday');

        return '<span title="' + dd1 + ' ' + time2 + '">' + dsp + '</span>';
    }

    function size (sizeStr) {
        if (!w2utils.isFloat(sizeStr) || sizeStr === '')
            return '';
        sizeStr = parseFloat(sizeStr);
        if (sizeStr === 0)
            return 0;
        var sizes = ['Bt', 'KB', 'MB', 'GB', 'TB'],
            i = parseInt(Math.floor(Math.log(sizeStr) / Math.log(1024)));
    ...
    function formatDate (dateStr, format) { // IMPORTANT dateStr HAS TO BE valid JavaScript Date String
        var months = w2utils.settings.shortmonths,
            fullMonths = w2utils.settings.fullmonths;
        ...
        if (w2utils.isInt(dateStr))
            dt = new Date(Number(dateStr)); // for unix timestamps
        var tmp = String(dateStr).split('-');
        if (tmp.length === 3)
            dt = new Date(tmp[0], Number(tmp[1]) - 1, tmp[2]); // yyyy-mm-dd
        var tmp = String(dateStr).split('/');
        if (tmp.length === 3)
            dt = new Date(tmp[2], Number(tmp[0]) - 1, tmp[1]); // mm/dd/yyyy
        if (dt === 'Invalid Date')
            return '';
    ...
    function formatTime (dateStr, format) { // IMPORTANT dateStr HAS TO BE valid JavaScript Date String
        ...
        if (typeof format === 'undefined')
            format = this.settings.time_format;
        if (typeof dateStr === 'undefined' || dateStr === '' || dateStr === null)
            return '';
    ...
    function transition (div_old, div_new, type, callBack) {
        ...
        setTimeout(function () {
            ...
            if (div_old) {
                ...
                if (div_old.parentNode)
                    $(div_old.parentNode).css({
                        '-webkit-perspective': '',
                        '-moz-perspective': '',
                        '-ms-perspective': '',
                        '-o-perspective': ''
        ...
        function cross (property, value, none_webkit_value) {
            ...
            if (!isWebkit && typeof none_webkit_value !== 'undefined')
                value = none_webkit_value;
    ...
    function lock (box, msg, spinner) {
        ...
        setTimeout(function () {
            var lock = $(box).find('.w2ui-lock'),
                mess = $(box).find('.w2ui-lock-msg');
            lock.data('old_opacity', lock.css('opacity')).css('opacity', '0').show();
            mess.data('old_opacity', mess.css('opacity')).css('opacity', '0').show();
            setTimeout(function () {
                var lock = $(box).find('.w2ui-lock'),
                    mess = $(box).find('.w2ui-lock-msg'),
                    left = ($(box).width() - w2utils.getSize(mess, 'width')) / 2,
                    top = ($(box).height() * 0.9 - w2utils.getSize(mess, 'height')) / 2;
                lock.css({
                ...
                if (!options.msg)
                    mess.css({'background-color': 'transparent', 'border': '0px'});
                if (options.spinner === true)
                    options.msg = '<div class="w2ui-spinner" ' + (!options.msg ? 'style="width: 30px; height: 30px"' : '') + '></div>' + options.msg;
    ...
    function scrollBarSize () {
        ...
        if (String(navigator.userAgent).indexOf('MSIE') >= 0)
            tmp.scrollBarSize = tmp.scrollBarSize / 2; // need this for IE9+
        return tmp.scrollBarSize;
    }
})();
...
w2utils.event = {
    ...
    trigger: function (eventData) {
        ...
        if (typeof eventData.target === 'undefined')
        ...
        for (var h = this.handlers.length - 1; h >= 0; h--) {
            ...
            if ((item.event.type === eventData.type || item.event.type === '*')
                ...
                if (eventData.isStopped === true || eventData.stop === true)
                    return eventData; // back compatibility eventData.stop === true
            }
        }
        // main object events
        ...
        // execute onComplete
        if (eventData.phase === 'after' && eventData.onComplete !== null)
...
w2utils.keyboard = (function (obj) {
    ...
    function keydown (event) {
        ...
        if ($.inArray(tag, ['INPUT', 'SELECT', 'TEXTAREA']) !== -1)
            return;
        if ($(event.target).prop('contenteditable') === 'true')
            return;
        if (!w2ui_name)
            return;
    ...
    function active (new_w2ui_name) {
        if (typeof new_w2ui_name === 'undefined')
            return w2ui_name;
...
(function () {
    ...
    $.fn.w2render = function (name) {
        if ($(this).length > 0) {
            if (typeof name === 'string' && w2ui[name])
                w2ui[name].render($(this)[0]);
    ...
    $.fn.w2tag = function (text, options) {
        ...
        return $(this).each(function (index, el) {
            ...
            if (text === '' || text === null || typeof text === 'undefined') {
                ...
                var originalCSS = '';
                if ($(el).length > 0)
                    originalCSS = $(el)[0].style.cssText;
                // bind event to hide it
                function tmp_hide () {
                    if ($('#w2ui-tag-' + tagID).length <= 0)
                        return;
                    clearInterval($('#w2ui-tag-' + tagID).data('timer'));
                    $('#w2ui-tag-' + tagID).remove();
                    $(el).off('keypress', tmp_hide).removeClass(options['class']);
                    if ($(el).length > 0)
                        $(el)[0].style.cssText = originalCSS;
    ...
    $.fn.w2overlay = function (html, options) {
        ...
        div = $('#w2ui-overlay');
        if (typeof bc !== 'undefined' && bc !== 'rgba(0, 0, 0, 0)' && bc !== 'transparent')
            div.css('background-color', bc);
        ...
        function fixSize () {
            ...
            if ($('#w2ui-overlay > div').length > 0) {
                ...
                if (h > max)
                    $('#w2ui-overlay> div').height(max).width(w + w2utils.scrollBarSize()).css({'overflow-y': 'auto'});
                // check width
    ...
    $.fn.w2menu = function (menu, options) {
        if (typeof options.select === 'undefined' && typeof options.onSelect === 'function')
            options.select = options.onSelect;
        ...
        function getMenuHTML () {
            ...
            for (var f = 0; f < menu.length; f++) {
                ...
                if (typeof mitem === 'string') {
                    ...
                    if (typeof mitem.text !== 'undefined' && typeof mitem.id === 'undefined')
                        mitem.id = mitem.text;
                    if (typeof mitem.text === 'undefined' && typeof mitem.id !== 'undefined')
                        mitem.text = mitem.id;
                    if (typeof mitem.caption !== 'undefined')
                        mitem.text = mitem.caption;
                    if (typeof mitem.img === 'undefined')
                        mitem.img = null;
                    if (typeof mitem.icon === 'undefined')
                        mitem.icon = null;
                }
                var img = '<td>&nbsp;</td>';
                if (mitem.img)
                    img = '<td><div class="w2ui-tb-image w2ui-icon ' + mitem.img + '"></div></td>';
                if (mitem.icon)
                    img = '<td align="center"><div class="w2ui-tb-image"><span class="' + mitem.icon + '"></span></div></td>';
                menu_html +=
