jQuery(function () {
    ...
    $('#changeTab').on('click', changeTab)
    $('#insertTab').on('click', insertTab)
});

function changeTab() {
    w2ui.tabs.get('tab2').text = `<div style="width: 200px; border: 1px solid red;">Tab ${(new Date).getTime()}</div>`;
    // w2ui.tabs.scroll('right')
    w2ui.tabs.refresh();
}

function insertTab() {
    w2ui.tabs.insert('new', { id: 'tab-'+(new Date()).getTime(), text: `Tab ${(new Date).getTime()}`, closable: true });
    w2ui.tabs.scroll('right')
}
